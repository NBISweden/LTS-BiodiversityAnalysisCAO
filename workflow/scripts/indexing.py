...
if method == "bwa-mem2":
    ...
    with open(f"{temp_folder}/index", "w") as handle:
        pass 
if method == "bowtie2":
    call(f"bowtie2-build --threads {threads} {temp_folder}/ref.fna {temp_folder}/index >> {logfile}  2>&1", shell=True)
    with open(f"{temp_folder}/index", "w") as handle:
        pass 
if method == "bbmap.sh":
    pass
if method == "minimap2":
       call(f"minimap2 -I 30G -t {threads} -d {temp_folder}/index {temp_folder}/ref.fna", shell=True)

title2log("Cleaning up and moving the index", logfile)

for f in os.listdir(temp_folder):
    if f.startswith("index"):
        shutil.copy(pjoin(temp_folder, f), pjoin(out_folder, f"results/mappings/{map_name}/"))
