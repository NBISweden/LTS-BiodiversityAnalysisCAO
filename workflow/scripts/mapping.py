...
for f in os.listdir(pjoin(out_folder, f"results/mappings/{map_name}/")):
    if f.startswith("index"):
        shutil.copy(
            pjoin(out_folder, f"results/mappings/{map_name}/", f), pjoin(temp_folder)
        )
...
call(
    f"samtools fastq -@ {threads} {temp_folder}/mapping_filtered.bam -1 {temp_folder}/matched_fwd.fastq -2 {temp_folder}/matched_rev.fastq -s {temp_folder}/matched_unp.fastq  2>> {logfile}",
    shell=True,
)
...
shutil.copy(
    f"{temp_folder}/matched_fwd.fastq",
    f"{out_folder}/results/mappings/{map_name}/{sample}_fwd.fastq",
)
shutil.copy(
    f"{temp_folder}/matched_rev.fastq",
    f"{out_folder}/results/mappings/{map_name}/{sample}_rev.fastq",
)
shutil.copy(
    f"{temp_folder}/matched_unp.fastq",
    f"{out_folder}/results/mappings/{map_name}/{sample}_unp.fastq",
)
